  ...
  public function storeReferats($data, $edition) {
    ...
    try {
      ...
      for ($i = 1; $i < count($data); $i++) {
        ...
        if (count($dataRow) == 4) {
          $this->$connection->prepare("INSERT INTO REF_LIBRARY (RefID, Title, Ref, Keywords, Max_Exports, CourseEdition) VALUES (?, ?, ?, ?, ?, ?)")
             ->execute(array($dataRow[0], $dataRow[1], $dataRow[2], $dataRow[3], $this->maxExportsPerReferat, $edition));
        }
      }
